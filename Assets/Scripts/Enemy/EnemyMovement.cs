...
public class EnemyMovement : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            collision.gameObject.SetActive(false);      //Destroy the object that collided with this object
        }
        ...
        if (collision.gameObject.tag == "Obstacle2")     //if collision with object that has the Obstacle2 tag occurs
        { 
            myRigidbody.velocity = new Vector2(myRigidbody.velocity.x, jumpforce2);      //move the object's rigidbody, with given jumpForce (Jump higher) 
        }
    }
}